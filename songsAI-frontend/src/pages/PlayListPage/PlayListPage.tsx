...
type MockCategoryKey = keyof typeof mockCategoryVideos;

const isMockCategoryKey = (emotion: string): emotion is MockCategoryKey => {
  return emotion in mockCategoryVideos;
};
...
const PlayListPage = () => {
  ...
  const handleCategoryClick = async (emotion: string) => {
    ...
    if(!isLogin){
      setCategoryVideos(
        isMockCategoryKey(emotion) ? mockCategoryVideos[emotion] : []
      );
      return;
    }
